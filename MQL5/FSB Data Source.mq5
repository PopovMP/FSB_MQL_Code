...
InstrumentProperties::InstrumentProperties(string symbol)
  {
   ...
   this.LotSize         = (int) SymbolInfoDouble(symbol, SYMBOL_TRADE_CONTRACT_SIZE);
   this.Slippage        = 0;
   this.Point           = SymbolInfoDouble(symbol,SYMBOL_POINT);
   this.Spread          = (tick.ask-tick.bid)/this.Point;
   this.SwapLong        = SymbolInfoDouble(symbol, SYMBOL_SWAP_LONG);
   this.SwapShort       = SymbolInfoDouble(symbol, SYMBOL_SWAP_SHORT);
   this.Commission      = 0;
   ...
   this.Digits          = (int) SymbolInfoInteger(symbol,SYMBOL_DIGITS);
   this.IsFiveDigits    = (this.Digits==3 || this.Digits==5);
   this.Pip             = IsFiveDigits ? 10*this.Point : this.Point;
   this.StopLevel       = (int) SymbolInfoInteger(symbol,SYMBOL_TRADE_STOPS_LEVEL);
   this.TickValue       = SymbolInfoDouble(symbol,SYMBOL_TRADE_TICK_VALUE);
   this.MinLot          = SymbolInfoDouble(symbol,SYMBOL_VOLUME_MIN);
   this.MaxLot          = SymbolInfoDouble(symbol,SYMBOL_VOLUME_MAX);
   this.LotStep         = SymbolInfoDouble(symbol,SYMBOL_VOLUME_STEP);
   this.MarginRequired  = SymbolInfoDouble(symbol,SYMBOL_MARGIN_INITIAL);
   if(this.MarginRequired<0.0001)
      this.MarginRequired=tick.bid*this.LotSize/100;
  }
//+------------------------------------------------------------------+
double InstrumentProperties::GetRate(string currProfit,string currAccount)
  {
   ...
   MqlTick tick;
   SymbolInfoTick(currAccount+currProfit,tick);
   double rate=tick.bid;
